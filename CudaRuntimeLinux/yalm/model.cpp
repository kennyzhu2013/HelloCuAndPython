...
// 这个头文件包含了多种用于处理向量和并行计算的内联函数和数据类型，支持不同的 SIMD 扩展，如 SSE、AVX 和 AVX2
// 定义了一些特殊的数据类型，如 __m128, __m256, __m512 等，这些类型用于表示向量数据
#include "immintrin.h"  //  C 和 C++ 中的一个头文件，主要用于提供对 Intel 的 SIMD（单指令多数据）指令集的支持
...
void* check_tensor(const Tensor* tensor, DType weight_dtype, std::array<int, 4> shape) {
    ...
    if (tensor->dtype != weight_dtype || tensor->shape != shape) {
        ...
        std::cerr
            << std::format("expected: dtype={}, shape=[{},{},{},{}]", dtype_to_string(weight_dtype), shape[0], shape[1], shape[2], shape[3])
            << std::endl;
        std::cerr
            << std::format("got: dtype={}, shape=[{},{},{},{}]", dtype_to_string(tensor->dtype), tensor->shape[0], tensor->shape[1], tensor->shape[2], tensor->shape[3])
            << std::endl;
        assert(false);
    }
    return tensor->data;
...
Model::Model(YALMData& yalm, int context) {
    ...
    for (int i = 0; i < config->n_layers; ++i) {
        blocks.emplace_back(std::make_shared<Block>(
            ...
            config->n_experts > 0 ? get_tensor(yalm, std::format("model.layers.{}.moegate.weight", i)) : nullptr
            ));
    }
