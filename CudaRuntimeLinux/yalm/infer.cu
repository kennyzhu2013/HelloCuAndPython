...
// __CUDACC__ 是一个预定义的宏，用于指示编译器当前正在编译 CUDA 代码。它主要用于区分 CUDA 代码和普通的 C/C++ 代码
#ifndef __CUDACC__
#define __CUDACC__
#endif
...
#define STATIC_KERNEL(x) if (!s.graph().is_created) x;
  ...
  half* vb = (half*)value_cache();
  {
    ...
    std::string resultLayer = std::to_string(_layer_i) + ":fused_rope_and_cache_update";
    s.graph().add_or_update_kernel_node(resultLayer, params, s.stream());
  }
  if (kv_sink > 0) {
    ...
    params.kernelParams = kernelParams;
    params.extra = nullptr;
    std::string resultLayer = std::to_string(_layer_i) + ":rotate_sink_tokens";
    s.graph().add_or_update_kernel_node(resultLayer, params, s.stream());
  }
  ...
  // multihead attention: dot products and softmax
  {
...

    {
      ...
      params.kernelParams = kernelParams;
      params.extra = nullptr;
      std::string resultLayer = std::to_string(_layer_i) + ":attn_dot";
      s.graph().add_or_update_kernel_node(resultLayer, params, s.stream());
    }

    {
      ...
      params.kernelParams = kernelParams;
      params.extra = nullptr;
      std::string resultLayer = std::to_string(_layer_i) + ":attn_softmax";
      s.graph().add_or_update_kernel_node(resultLayer, params, s.stream());
    }
  }
  // multihead attention: mix values with attention scores
  {
    ...
    params.kernelParams = kernelParams;
    params.extra = nullptr;
    std::string resultLayer = std::to_string(_layer_i) + ":att_mix";
    s.graph().add_or_update_kernel_node(resultLayer, params, s.stream());
  }
